...
async function main() {
  ...
  const trainingModules = [
    {
      ...
      content: {
        sections: [
          { title: 'Core Capabilities', items: ['Digital Twin', 'Generative Reasoning', 'Auto-Correction', 'Explainable AI'] },
          { title: 'Architecture Layers', items: ['Executive Experience Layer', 'Agent Orchestration', 'Intelligent Core', 'Unified Data Fabric'] },
        ],
      },
      resources: [
    ...
    },
    {
      ...
      content: {
        sections: [
          { title: 'Filtering & Segmentation', items: ['Region', 'Line of Business (LOB)', 'Vertical'] },
          { title: 'Key Features', items: ['Sales Funnel Analysis', 'Stalled Opportunities', 'Channel Breakdown'] },
        ],
      },
      resources: [],
    },
    {
      ...
      content: {
        sections: [
          { title: 'Key Features', items: ['License vs. Implementation Toggle', 'Margin by Segment', 'Account Distribution', 'Net Margin Trend'] },
        ],
      },
      resources: [],
    },
    {
      ...
      content: {
        sections: [
          { title: 'Key Features', items: ['Marketing Funnel', 'Lead Distribution', 'Channel Efficiency', 'Campaign Performance'] },
        ],
      },
      resources: [],
    },
    {
      ...
      content: {
        sections: [
          { title: 'Key Metrics', items: ['CAC Payback Period', 'LTV:CAC Ratio', 'Net Revenue Retention', 'Magic Number', 'Rule of 40', 'Cost Per Lead'] },
        ],
      },
      resources: [],
    },
    {
      ...
      content: {
        sections: [
          { title: 'Revenue Waterfall', items: ['Base Contracted', 'New Pipeline', 'Churn', 'Final Forecast'] },
          { title: 'Product Profitability', items: ['Contribution Margin analysis by product line'] },
        ],
      },
      resources: [],
    },
    {
      ...
      content: {
        sections: [
          { title: 'Phantom Cost Hunter', items: ['Duplicate SaaS licenses', 'Unused seats', 'T&E Policy Violations'] },
          { title: 'Vendor Spend Analysis', items: ['Treemap visualization', 'YoY growth tracking'] },
        ],
      },
      resources: [],
    },
    {
      ...
      content: {
        sections: [
          { title: 'Model Drift Detection', items: ['Accuracy monitoring', 'Auto-retraining pipeline'] },
          { title: 'Active Models', items: ['Revenue Forecasting (94.3%)', 'Anomaly Detection (96.7%)', 'Churn Prediction (89.1%)', 'Driver Analysis (91.2%)'] },
        ],
      },
      resources: [],
    },
    {
      ...
      content: {
        sections: [
          { title: 'Simulation', items: ['10,000 Monte Carlo simulations', 'Probability distribution outputs'] },
          { title: 'GenAI Analysis', items: ['Mitigation Strategies', 'Growth Opportunities'] },
        ],
      },
      resources: [],
    },
    {
      ...
      content: {
        sections: [
          { title: 'Key Features', items: ['Data Lineage Explorer', 'Audit Trail', 'SOX Compliance'] },
        ],
      },
      resources: [],
    },
    {
      ...
      description: 'CSV Templates and Import Best Practices',
      category: 'fundamentals',
      duration: '10 min',
      sortOrder: 11,
      content: {
        sections: [
          { title: 'Templates', items: ['Financial Metrics', 'Profitability Data', 'Sales Performance', 'Revenue Analytics', 'Cost Data', 'Vendors', 'Cost Centers'] },
          { title: 'Best Practices', items: ['Download templates first', 'Use consistent naming', 'Date format: YYYY-MM-DD', 'No currency symbols', 'Max 50MB file size'] },
        ],
      },
      resources: [],
    },
  ...
  const fundamentalsModules = await prisma.trainingModule.findMany({
    where: { category: 'fundamentals' },
  });
  const advancedModules = await prisma.trainingModule.findMany({
  ...
  const certificates = [
    {
      ...
      validityDays: 365,
      moduleIds: fundamentalsModules.map((m) => m.id),
    },
    {
      title: 'Advanced Analytics',
      description: 'Master advanced analytics and AI capabilities',
      validityDays: 365,
      moduleIds: advancedModules.map((m) => m.id),
    },
    {
      title: 'Aether Platform Expert',
      description: 'Complete all training modules for full certification',
      validityDays: 730,
